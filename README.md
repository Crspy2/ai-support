...
- **Info collection modals** — when an action needs data (e.g. a loader username), the bot asks for it privately via a Discord modal
...
request_info(
  ...
  message = "I need your loader username and a license key to verify your identity.",
  fields = [
    { id = "username", label = "Loader Username" },
    { id = "key", label = "License Key" }
  ],
  then = "PanelExtension::link_account"
)
```
...
| `linked_accounts` | Discord ↔ loader account links (verified by key) |
| `reset_attempts` | HWID/password reset audit log with cooldowns |
...
│   └── panel_extension.rs     # Panel: account lookup, HWID/password reset, linking
├── knowledge/
...
└── db/
    ...
    └── migrations/
        ...
        ├── 005_linked_accounts.sql  # linked_accounts
        └── 006_reset_attempts.sql   # reset_attempts (with pg enum)
